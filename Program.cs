...
await ConsoleApp.RunAsync(args, Main);
...
partial class Program
{
    ...
    static async Task Main(string? host = null, int? port = null, [Arg] IPEndPoint? ip = null, [Arg] Uri? uri = null, string? hex = null, string? str = null, string? encoding = null, bool showEncodings = false)
    {
        if (showEncodings)
        {
...
class ArgAttribute : Attribute,
    IArgumentParser<IPEndPoint?>,
    IArgumentParser<Uri?>
{
    public static bool TryParse(ReadOnlySpan<char> s, out IPEndPoint? result) => IPEndPoint.TryParse(s, out result);
    public static bool TryParse(ReadOnlySpan<char> s, out Uri? result) => Uri.TryCreate(new(s), uriKind: UriKind.Absolute, out result);
}